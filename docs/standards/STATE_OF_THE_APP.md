...
- **Portfolio status (2026-09-18):** Public repository, work in progress; no official release or hosted deployment. Full-text reading and browser publication are implemented on the showcase branch; learner progress remains incomplete. The security fix passes local image scans and PR checks; main-branch scanned-artifact publication is pending. Hosting/provider selection is explicitly deferred; use the isolated local demo guide in `ops/local-demo/README.md`.
...
- **E2E:** Playwright suite in `/e2e` (7 journeys): OAuth2 Code+PKCE login/logout, role-gated authoring navigation, creation/validation, feed pagination, and browser author → curator → learner with denied draft access and shared-link authentication. See `ops/local-demo/README.md` for local execution.
- **Test coverage:** ≥85% line+branch per module enforced at build. Backend integration/architecture/coverage gates passed in the security validation. Frontend: 161 Vitest tests; measured 99.29% statements / 90.90% branches / 100% functions / 99.60% lines for the reader/curation change (85% gate retained).
...
## [2026-09-18] Complete browser reading/publication and document the local showcase
- **Agent/Author:** Codex
- **Task:** Deliver the browser catalog journey (#2) and reproducible local showcase (#3); prepare local execution while hosting stays deferred.
- **Changes:** Added authenticated pill detail, safe plain-text/block rendering with JSON fallback, curator publication with detail/feed cache updates, loading/error/not-found handling, feed and draft links, and safe detail-route preservation through OIDC state. Added an isolated Compose overlay and walkthrough, plus real desktop/mobile screenshots in README. Rich content interactions and completion are explicitly not claimed.
- **Standards compliance:** Regression tests written first; missing detail and OIDC-return behavior failed before implementation. Browser journey failed on the previous image at the missing draft link, then passed on the new image. Publication has a use-case span; existing API authorization, ownership, structured logs and business metrics remain authoritative. No executable HTML, token persistence or new production dependency added.
- **Tests:** 161 Vitest tests passed, statements 99.29% / branches 90.90%; lint and TypeScript passed. Prettier passed with Windows line endings accepted (the default local check flags pre-existing CRLF files; Linux CI retains the standard gate). Docker build passed. Seven real-stack Playwright journeys passed, 23.5s/no retries. Compose config validated. Desktop/mobile screenshots inspected; no horizontal overflow at 1440px/390px.
- **Follow-ups / debt:** Changes await PR review/merge. #4 hosting/edge validation, #5 governance alignment, #6 learner completion remain open. Text/Markdown is plain text and unsupported block types are JSON, not interactive renderers. Local synthetic fixtures only; no production deployment or release.
...
- **Changes:** Revised README status, scope and delivery limitations; registered CodePill issues [#1](https://github.com/pedrovvitor/CodePill/issues/1)–[#6](https://github.com/pedrovvitor/CodePill/issues/6). Also updated the two public companion repositories' README/description and registered Talent Intelligence #48–#52. Preserved existing wexchange issues rather than duplicating them. Changed CodePill visibility to public as requested, verified through the GitHub API, and pinned all three projects on the profile. Personal profile visibility remains a separate user preference.
